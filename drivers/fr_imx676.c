...
static const char * const imx676_conversion_gain_menu[] = {
...
static void imx676_adjust_gain_range(struct imx676 *imx676, u8 val)
{
	if (val) {
		__v4l2_ctrl_modify_range(imx676->gain,
					IMX676_CONVERSION_ANA_GAIN_MIN,
					IMX676_ANA_GAIN_MAX, 1,
					IMX676_CONVERSION_ANA_GAIN_DEFAULT);

		__v4l2_ctrl_s_ctrl(imx676->gain, IMX676_CONVERSION_ANA_GAIN_DEFAULT);
	}
		...
		__v4l2_ctrl_s_ctrl(imx676->gain, IMX676_ANA_GAIN_DEFAULT);
	}
...
static int imx676_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_CONVERSION_GAIN:
		imx676_adjust_gain_range(imx676, ctrl->val);
		break;
	}
	...
	switch (ctrl->id) {
	case V4L2_CID_CONVERSION_GAIN:
		ret = imx676_write_hold_reg(imx676, FDG_SEL0, 1, ctrl->val);
		break;
...
static struct v4l2_ctrl_config imx676_ctrl_conversion_gain[] = {
	{
		.ops = &imx676_ctrl_ops,
		...
		.qmenu = imx676_conversion_gain_menu,
	},
};
...
static int imx676_init_controls(struct imx676 *imx676)
{
	...
	imx676->gain = v4l2_ctrl_new_std(ctrl_hdlr, &imx676_ctrl_ops,
	...
	imx676->conversion_gain = v4l2_ctrl_new_custom(ctrl_hdlr,
					imx676_ctrl_conversion_gain, NULL);
...
static int imx676_check_hwcfg(struct device *dev, struct i2c_client *client)
{
	...
	*/
	imx676->no_reset_gpio = of_property_read_bool(node, "no-reset-gpio");
...
static int imx676_probe(struct i2c_client *client)
{
	...
	if (strcmp(imx676->gmsl, "gmsl") && !imx676->no_reset_gpio) {
